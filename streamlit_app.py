...
def main():
    ...
    st.markdown(
        "Upload images and analyze them using Azure OpenAI GPT-4o vision capabilities."
    )
    ...
    with st.sidebar:
        ...
        st.info(
            "The system prompt defines the AI's role and behavior for all image analyses."
        )
        ...
        if uploaded_files:
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                with st.expander(f"📷 {uploaded_file.name}", expanded=True):
                    ...
                    # Image info
                    st.caption(
                        f"Size: {image.size[0]}x{image.size[1]} pixels | Format: {image.format}"
                    )
    ...
    with col2:
        ...
        if uploaded_files:
            # Analyze button
            if st.button(
                "🔍 Analyze All Images", type="primary", use_container_width=True
            ):
                ...
                for i, uploaded_file in enumerate(uploaded_files):
                    ...
                    status_text.text(
                        f"Analyzing {uploaded_file.name}... ({i+1}/{len(uploaded_files)})"
                    )
